...
class PYDDT(App):
    ...
    def build(self):
        ...
        layout.add_widget(Button(text= 'SCAN', id='scan', size_hint=(1, None), on_press=self.scanALLecus, height=(fs * 4,  'dp')))
    ...
    def finish(self, instance):
        if instance == 'scan': mod_globals.opt_scan = True
    ...
    def in_car(self):
        ...
        for avto in self.avtosd:
            btn = Button(text=avto['name'], size_hint_y=None, height=(fs * 3,  'dp'))
    ...
    def popup_in_car(self, text, avto=[]):
            ...
            for avto in self.avtosd:
                for car in avto['list']:
                    if self.find.text.lower() in str(car).lower():
                        btn = Button(text=car['code']+' : '+car['name'], height=(fs * 3,  'dp'), size_hint_y=None)
        ...
        """
                ...
                for car in avto.values()[0]:
                    if self.find.text.lower() in str(car).lower():
                        btn = Button(text=car['code']+' : '+car['name'], height=(fs * 3,  'dp'), size_hint_y=None)
                        layout.add_widget(btn)
                        btn.bind(on_release=lambda btn: self.press_car(btn.text))
            if avto.keys()[0] == instance:
                for car in avto.values()[0]:
                    btn = Button(text=car['code']+' : '+car['name'], height=(fs * 3,  'dp'), size_hint_y=None)
                    layout.add_widget(btn)
                    btn.bind(on_release=lambda btn: self.press_car(btn.text))"""
        ...
        root.add_widget(layout)
        self.popup = Popup(title=text, content=root, size=(Window.size[0]*0.8, Window.size[1]*0.8), size_hint=(None, None), auto_dismiss=True)
        self.popup.open()
